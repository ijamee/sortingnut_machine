...
int i = 0; //The new Wire1 library needs to take an int when you are sending for the zero register
...
#if (ARDUINO >= 100)
	#define WW_  Wire1.write
	#define WR_  Wire1.read
#else
	#define WW_  Wire1.send
	#define WR_  Wire1.receive
#endif	
...
uint8_t RTC_DS1307::readMemory(uint8_t offset, uint8_t* data, uint8_t length) {
  ...
  Wire1.requestFrom((uint8_t)DS1307_ADDRESS, (uint8_t)length);
  while (Wire1.available() > 0 && bytes_read < length) {
    data[bytes_read] = WR_();
    bytes_read++;
  }
...
DateTime RTC_DS3231::now()
{
    ...
    Wire1.requestFrom(DS3231_ADDRESS, 19);
...
float RTC_DS3231::getTemperature() {
    ...
    temp = Wire1.RECEIVE();  // Here's the MSB
    return float(temp) + 0.25*(Wire1.RECEIVE()>>6);
}
...
void RTC_DS3231::getA1Time(byte& A1Day, byte& A1Hour, byte& A1Minute, byte& A1Second, byte& AlarmBits, bool& A1Dy, bool& A1h12, bool& A1PM) {
    ...
    temp_buffer = Wire1.RECEIVE();   // Get A1M1 and A1 Seconds
    ...
    temp_buffer     = Wire1.RECEIVE();   // Get A1M2 and A1 minutes
    ...
    temp_buffer = Wire1.RECEIVE();   // Get A1M3 and A1 Hour
    ...
    temp_buffer = Wire1.RECEIVE();   // Get A1M4 and A1 Day/Date
...
void RTC_DS3231::getA2Time(byte& A2Day, byte& A2Hour, byte& A2Minute, byte& AlarmBits, bool& A2Dy, bool& A2h12, bool& A2PM) {
    ...
    temp_buffer = Wire1.RECEIVE();   // Get A2M2 and A2 Minutes
    ...
    temp_buffer = Wire1.RECEIVE();   // Get A2M3 and A2 Hour
    ...
    temp_buffer = Wire1.RECEIVE();   // Get A2M4 and A1 Day/Date
...
void RTC_DS3231::setA1Time(byte A1Day, byte A1Hour, byte A1Minute, byte A1Second, byte AlarmBits, bool A1Dy, bool A1h12, bool A1PM) {
    ...
    byte temp_buffer;
    Wire1.beginTransmission(DS3231_ADDRESS);
    Wire1.SEND(0x07);    // A1 starts at 07h
    // Send A1 second and A1M1
    Wire1.SEND(bin2bcd(A1Second) | ((AlarmBits & 0b00000001) << 7));
    // Send A1 Minute and A1M2
    Wire1.SEND(bin2bcd(A1Minute) | ((AlarmBits & 0b00000010) << 6));
    // Figure out A1 hour 
    if (A1h12) {
...
void RTC_DS3231::setA2Time(byte A2Day, byte A2Hour, byte A2Minute, byte AlarmBits, bool A2Dy, bool A2h12, bool A2PM) {
    ...
    byte temp_buffer;
    Wire1.beginTransmission(DS3231_ADDRESS);
    Wire1.SEND(0x0b);    // A1 starts at 0bh
    // Send A2 Minute and A2M2
    Wire1.SEND(bin2bcd(A2Minute) | ((AlarmBits & 0b00010000) << 3));
    // Figure out A2 hour 
    if (A2h12) {
        // Start by converting existing time to h12 if it was given in 24h.
        if (A2Hour > 12) {
